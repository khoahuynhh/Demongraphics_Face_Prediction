...
class Config:
    ...
    OUTPUT_DIR = "./outputs/yolo26n_cls"  # Output directory for the model

    # Model parameters
    BACKBONE = "yolo26n_cls"
    USE_PRETRAINED = True  # Load Ultralytics ImageNet pretrained classification weights
    EMBEDDING_SIZE = None
...
def freeze_layers(model, freeze_until="layer3", verbose=True):
    if (
        hasattr(model.backbone, "backbone_name")
        and model.backbone.backbone_name in YOLO_CLS_WEIGHTS
    ):
        for p in model.backbone.parameters():
            p.requires_grad = False
        if verbose:
            print("[freeze] Frozen full Ultralytics YOLO classification backbone")
        return
...
YOLO_CLS_WEIGHTS = {
    ...
    "yolo26s_cls": "yolo26s-cls.pt",
    "yolo26m_cls": "yolo26m-cls.pt",
    "yolo26l_cls": "yolo26l-cls.pt",
    "yolo26x_cls": "yolo26x-cls.pt",
    "yolo11n_cls": "yolo11n-cls.pt",
    "yolo11s_cls": "yolo11s-cls.pt",
    "yolo11m_cls": "yolo11m-cls.pt",
    "yolo11l_cls": "yolo11l-cls.pt",
    "yolo11x_cls": "yolo11x-cls.pt",
    "yolov8n_cls": "yolov8n-cls.pt",
    "yolov8s_cls": "yolov8s-cls.pt",
}


class UltralyticsClsBackbone(nn.Module):
    def __init__(self, model_name="yolo26n-cls.pt", pretrained=True):
        super().__init__()

        weight_name = model_name if pretrained else model_name.replace(".pt", ".yaml")
        yolo = YOLO(weight_name)

        # Internal PyTorch model from Ultralytics
        core = yolo.model

        # The last layer is usually the classifier head, so remove it to get features
        if hasattr(core, "model"):
            layers = list(core.model.children())
            self.features = nn.Sequential(*layers[:-1])
        else:
            raise ValueError("Cannot access Ultralytics classification backbone.")

        self.pool = nn.AdaptiveAvgPool2d((1, 1))
        self.feature_dim = self._infer_feature_dim(Config.IMG_SIZE)

    def _infer_feature_dim(self, img_size):
...
class FaceBackbone(nn.Module):
    def __init__(
        self, backbone_name="yolo26n_cls", pretrained: bool = True, device="cpu"
    ):
        super().__init__()

        self.backbone_name = backbone_name

        if backbone_name in YOLO_CLS_WEIGHTS:
            self.backbone = UltralyticsClsBackbone(
                YOLO_CLS_WEIGHTS[backbone_name],
    ...
    def forward(self, x):
        if self.backbone_name in YOLO_CLS_WEIGHTS:
...
class FaceAttrModel(nn.Module):
    def __init__(self, backbone_name="yolo26n_cls", pretrained=True, emo_classes=7):
        super().__init__()
    ...
    def forward(self, x):
        ...
        age = self.age_head(feat).squeeze(1) * 100
        gender_logits = self.gender_head(feat)
        emotion_logits = self.emo_head(feat)
...
def train_one_epoch_attr(model, loader, optimizer, device):
    ...
    for imgs, age, gender, emo in loader:
        ...
        m_age = age != -1
        if m_age.any():
            # Compute loss only for samples that have age labels
            loss_age_raw = age_loss_fn(out["age"][m_age].squeeze(), age[m_age])
            # Apply learnable weighting to the age task
        ...
        # --- TOTAL LOSS ---
        # Only tasks with labels in the current batch contribute to loss and gradients
...
def main():
    ...
    # Model information
    print(f"Model: {Config.BACKBONE} with feature dim {model.backbone.feature_dim}")
...
def export_faceattr_to_onnx(
    ...
    model = FaceAttrModel(
        ...
        pretrained=True,  # build the matching Ultralytics architecture before loading weights
    ...
    # 3) Load state dict. Keep strict by default so a wrong checkpoint cannot
    # silently export a partially random model.
